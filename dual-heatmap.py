...
def parse_arguments():
    ...
    parser.add_argument('-o', '--operations', type=int, help='Number of operations')
    parser.add_argument('-p', '--partials', type=int, nargs='+', help='partials list')
    parser.add_argument('-i', '--prefill', type=int, nargs='+', help='prefill list')
    parser.add_argument('--operation_json', type=str, help='Path to operation JSON file')
    parser.add_argument('--length_json', type=str, help='Path to length JSON file')
...
def run_rust_test(operations, partials, prefill):
    ...
    length_result = subprocess.run(base_command + " --heuristic length", capture_output=True, text=True, shell=True)
    if length_result.returncode != 0:
        print("Error running the Rust command")
        print(length_result.stderr)
        exit()

    operation_result = subprocess.run(base_command + " --heuristic operation", capture_output=True, text=True, shell=True)
...
def plot_heatmap(operation_data, length_data, titles=['Length Heuristic', 'Operation Heuristic']):
    fig, axes = plt.subplots(ncols=2, width_ratios=[0.45, 0.55], sharey=True, figsize=(6, 3))
    ...
    for ax_idx, (ax, data, title) in enumerate(zip(axes, data_sets, titles)):
        ...
        sns_heatmap = sns.heatmap(data_pivot, annot=False, fmt=".2f", ax=ax,
                                  xticklabels=y_unique, yticklabels=(x_unique if ax_idx == 1 else False),
                                  vmin=vmin, vmax=vmax, cbar=ax_idx == len(axes)-1)
        ax.set_title(title)
        ax.set_xlabel('prefill')
        if ax_idx == 0: ax.set_ylabel('partials')
...
def main():
    ...
    if args.operation_json and args.length_json:
        ...
        if not (args.operations and args.partials and args.prefill):
            print("Please provide operations, partials, and prefill arguments or specify JSON files.")
            return
        
        # Run Rust command to generate new JSON files
        operation_output, length_output = run_rust_test(args.operations, args.partials, args.prefill)
    ...
    # Parse and plot data
    operation_data_parsed, length_data_parsed = parse_and_transform_data(operation_data, length_data)
