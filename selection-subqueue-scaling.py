...
def run_rust_test(operations, subqueues, prefill, runs, readout, selection):
    subqueues_str = ' '.join(map(str, subqueues))
    out_name = selection.replace("-", "_")
    if selection != "random-unique":
        command = (
            f"cargo run -r -- subqueues-and-prefill -o {operations} -s {subqueues_str} -i {prefill} -r {runs}"
            f" --heuristic operation --readout {readout} --selection {selection} --output-name {out_name}"
        )
    else:
        command = (
            f"cargo run -r -- subqueues-and-prefill -o {operations} -s {subqueues_str} -i {prefill} -r {runs}"
            f" --heuristic operation --readout {readout} --selection random --sampling uniques --output-name {out_name}"
        )
...
def plot_graphs(data_points, save_path, error_column, title="Selection scheme impact"):
    fig, ax = plt.subplots(figsize=(4,3))

    for (scheme, data) in data_points.items():
        # Create a DataFrame from the list of tuples
        df = pd.DataFrame(data, columns=['sub-queues', error_column])

        sns.lineplot(data=df, x='sub-queues',
                     y=error_column, ax=ax, marker='o', label=scheme)
...
def main():
    ...
    if args.old_json:
        # Read data from the old folder with previous json
        data_points = {}
        for file in Path(args.old_json).iterdir():
            if file.suffix == ".json":
                data_points[file.stem.split("-")[0]] = read_and_parse_data(file)
    else:
        if not (args.operations and args.subqueues and args.prefill):
            print(
                "Please provide operations, subqueues, and prefill arguments.")
            return

        data_points = {}
        for selection in ["random", "left", "random-part", "left-part", "random-unique"]:
            # Run Rust command to generate new JSON files
            output = run_rust_test(
                args.operations, args.subqueues, args.prefill, args.runs, args.readout, selection)
            file_path = extract_file_path(output)
            data_points[selection] = read_and_parse_data(file_path)


    # Parse and plot data
    error_column = {"average": "Average Rank Error",
                    "worst-one-percent": "Worst 1% point Rank Error"}[args.readout]
    plot_graphs(data_points, args.save_path, error_column)
